...
@interface ListLightViewController ()<UITableViewDelegate,UITableViewDataSource,ZFTableViewCellDelegate>
...
@implementation ListLightViewController
...
- (void)requestData {
    ...
    for (NSDictionary *dataDic in videoList) {
        ...
        CellLayout *layout = [[CellLayout alloc] initWithData:data];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    [scrollView zf_filterShouldPlayCellWhileScrolling:^(NSIndexPath *indexPath) {
        if ([indexPath compare:self.tableView.zf_shouldPlayIndexPath] != NSOrderedSame) {
            @strongify(self)
            /// 显示黑色蒙版
            TableViewCell *cell1 = [self.tableView cellForRowAtIndexPath:self.tableView.zf_shouldPlayIndexPath];
            [cell1 showMaskView];
            /// 隐藏黑色蒙版
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    TableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kIdentifier];
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[TableViewCell class] forCellReuseIdentifier:kIdentifier];
